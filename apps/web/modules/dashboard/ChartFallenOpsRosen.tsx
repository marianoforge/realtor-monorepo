...
import { tiposOperacionesCaidasPieChartData } from "@gds-si/shared-utils";
...
const ChartFallenOpsRosen = () => {
  ...
  if (isMobile) {
    return (
      <MobileDonutChartView
        data={pieChartData}
        title="Operaciones Caídas"
        icon={<CircleStackIcon className="h-6 w-6 text-red-600" />}
        description="Operaciones que no se completaron"
      />
    );
  }

  // Chart dimensions - EXACTAMENTE IGUAL que CuadroPrincipalChartRosen
  const radius = Math.PI * 45;
  const gap = 0.02; // Gap between slices

  // Pie layout and arc generator - EXACTAMENTE IGUAL (DONUT)
  const pieLayout = pie<DataItem>()
    .sort(null)
    .value((d) => d.value)
    .padAngle(gap);

  // Create DONUT shape with inner radius - IGUAL que CuadroPrincipalChartRosen
  ...
  // Helper function para obtener el año de una operación de forma segura
  const getOperationYear = (op: Operation): number | null => {
    const fechaStr = op.fecha_operacion || op.fecha_reserva;
  ...
  // Calcular el total de operaciones cerradas del año efectivo
  // El porcentaje se calcula sobre operaciones cerradas (no sobre todas)
  const currentYear = effectiveYear;
  const totalOperaciones = operations.filter((op: Operation) => {
    const operationYear = getOperationYear(op);
    return (
      operationYear !== null &&
  ...
  // Calcular el porcentaje de operaciones caídas
  const porcentaje =
    totalOperaciones > 0
      ? ((total / totalOperaciones) * 100).toFixed(1)
      : "0.0";
  ...
  return (
    <div className="bg-white p-3 rounded-xl shadow-sm border border-gray-100 w-full h-[380px]">
      ...
      >
        {/* Donut Chart with centered text - EXACTAMENTE IGUAL que CuadroPrincipalChartRosen */}
...
export default ChartFallenOpsRosen;
